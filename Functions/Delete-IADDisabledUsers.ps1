Function Delete-IADDisabledUsers{
<#
.SYNOPSIS
...
#>       
    ...
    foreach($Account in $ExclusionList){
        if($FirstPass -like "1"){$FilterExclusionList += "samaccountname -notlike `"$Account`""}
        else{$FilterExclusionList += " -and samaccountname -notlike `"$Account`""}
        $FirstPass = "0"
    }
    $FilterString = "$FilterExclusionList"

    #Write message if AD Recycle Bin not enabled and switchparameter -NoRecycleBin not specified
    if( (!(Get-ADOptionalFeature -filter{name -like "Recycle Bin Feature"}).EnabledScopes) -and !($NoRecycleBin)){
        Write-Host "!!WARNING!! AD Recycle Bin is not enabled! Proceed with caution! Use -NoRecycleBin to ignore next time." -ForegroundColor Red -BackgroundColor Black
        sleep 10
